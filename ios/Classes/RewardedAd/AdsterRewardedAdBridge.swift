...
class AdsterRewardedAdBridge : NSObject{
    
    private var adClickChannel: FlutterMethodChannel
    private var adLoadChannel: FlutterMethodChannel
    private var ads: [String:AdsterRewardedAd] = [:]
    
    init(messenger : FlutterBinaryMessenger) {
        self.adLoadChannel = FlutterMethodChannel(name: "adster.channel:adster_rewarded_ad", binaryMessenger: messenger)
        self.adClickChannel = FlutterMethodChannel(name: "adster.channel:adster_rewarded_ad_click", binaryMessenger: messenger)
        super.init()
                ...
                if(args != nil){
                    ...
                    if(widgetId != nil){
                        ...
                        if(placement != nil){
                            ...
                            var interstitialAd: AdsterRewardedAd?
                            if(self.ads.keys.contains(widgetId ?? "")){
                                interstitialAd = self.ads[widgetId ?? ""] ?? nil
                            }else{
                                interstitialAd = AdsterRewardedAd(widgetId: widgetId ?? "", placementId: placement ?? "", adClickChannel: self.adClickChannel, adLoadChannel: self.adLoadChannel, publisherProvidedId: publisherProvidedId, customTargetingParams: customTargetingParams)
